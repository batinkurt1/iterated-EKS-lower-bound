...
estimated_covariances = zeros(4,4,simulation_length);

smoothed_states = zeros(4,simulation_length);
smoothed_covariances = zeros(4,4,simulation_length);
...
for k=1:simulation_length
    ...
    [estimated_states(:,k),estimated_covariances(:,:,k)] = EKF_measurement_update(predicted_state,predicted_covariance,predicted_measurement,actual_measurement,h,R,apply_numerical_derivative,dh_dx);
end
...
while state_diff > 1e-12
    ...
    for k=1:simulation_length
        ...
        [estimated_states(:,k),estimated_covariances(:,:,k)] = IEKF_measurement_update(predicted_state,predicted_covariance,predicted_measurement,actual_measurement,h,R,smoothed_states(:,k),apply_numerical_derivative,dh_dx);
    end
